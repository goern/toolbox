...
func initContainer(cmd *cobra.Command, args []string) error {
	...
	if initContainerFlags.monitorHost {
		...
		if utils.PathExists("/run/host/etc") {
			...
			if _, err := os.Readlink("/etc/host.conf"); err != nil {
				if err := redirectPath("/etc/host.conf", "/run/host/etc/host.conf", false);
				err != nil {
					return err
				}
			}
			...
			if _, err := os.Readlink("/etc/hosts"); err != nil {
				if err := redirectPath("/etc/hosts", "/run/host/etc/hosts", false); err != nil {
					return err
				}
			}

			if _, err := os.Readlink("/etc/resolv.conf"); err != nil {
				if err := redirectPath("/etc/resolv.conf", "/run/host/etc/resolv.conf", false);
				err != nil {
					return err
				}
			}
		...
		if utils.PathExists("/run/host/monitor") {
			...
			if localtimeTarget, err := os.Readlink("/etc/localtime");
			err != nil || localtimeTarget != "/run/host/monitor/localtime" {
				if err := redirectPath("/etc/localtime", "/run/host/monitor/localtime", false);
				err != nil {
					return err
				}
			}
			...
			if _, err := os.Readlink("/etc/timezone"); err != nil{
				if err := redirectPath("/etc/timezone", "/run/host/monitor/timezone", false);
				err != nil {
					return err
				}
			}
	...
	if _, err := user.Lookup(initContainerFlags.user); err != nil {
		...
		logrus.Debugf("Arguments passed to 'useradd': %v", useraddArgs)
		...
		passwdArgs := []string { "--delete", initContainerFlags.user }
		passwdCmd := exec.Command("passwd", passwdArgs...)
...
[libdefaults]
    default_ccache_name = KCM:`
		...
		if err := ioutil.WriteFile("/etc/krb5.conf.d/kcm_default_ccache", kcmConfigBytes, 0644);
		err != nil {
