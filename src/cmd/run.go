...
func run(cmd *cobra.Command, args []string) error {
	...
	var release string
	if runFlags.release > 0 {
		nonDefaultContainer = true
		release = fmt.Sprint(runFlags.release)
	}

	if len(args) == 0 {
		return errors.New("missing argument for \"run\"\nRun 'toolbox --help' for usage.")
	}
...
func runCommand(container string,
	...
	if _, err := podman.ContainerExists(container); err != nil {
		...
		if pedantic {
			err := utils.CreateErrorContainerNotFound(container)
			return err
		}
	...
	logrus.Debug(execArgs)

	podman.CmdInto(execArgs...)
...
func startContainer(container string) error {
	...
	if outputBytes, err := podman.CmdOutput(args...); err != nil {
		outputString := string(outputBytes)
		if strings.Contains(outputString, "use system migrate to mitigate") {
			...
			if err != nil {
				...
				fmt.Fprintf(&builder,
					"failed to migrate containers to OCI runtime %s\n",
					ociRuntimeRequired)

				fmt.Fprintf(&builder, "Factory reset with: podman system reset")
			}

			err = podman.CmdRun("start", container)
			if err != nil {
				var builder strings.Builder

				fmt.Fprintf(&builder,
					"container %s doesn't support cgroups %s\n",
					container,
					cgroupsVersion)
